<template>
    <button v-bind="$attrs" class='btn btn--default' :class="{ 'btn--loading': loading }" ref="button"
        :type="$attrs.type || 'button'" :disabled="isDisabled || isLoading" @click="handleClick"
        @focus="e => $emit('focus', e)" @blur="e => $emit('blur', e)">
        <v-icon v-if="prependIcon" class="mr-2">mdi-{{ prependIcon }}</v-icon>
        <slot v-if="$slots.default" />
        <span v-else>
            {{ name }}
        </span>
        <v-icon v-if="appendIcon" class="ml-2">mdi-{{ appendIcon }}</v-icon>
    </button>
</template>
